...
if ( ! function_exists( 'wilbur_generate_css' ) ) {
	 ...
	 */
	function wilbur_generate_css( $selector, $style, $value, $prefix = '', $suffix = '', $echo = true ) {
...
if ( ! function_exists( 'wilbur_get_customizer_css' ) ) {
		...
		*/
		...
		if ( 'front-end' === $type ) {
			...
			if ( $cover && $cover !== $cover_default ) {
				wilbur_generate_css( '.overlay-header .header-inner', 'color', $cover );
				wilbur_generate_css( '.cover-header .entry-header *', 'color', $cover );
			}
			...
			if ( $accent && $accent !== $accent_default ) {
				...
				wilbur_generate_css( '.editor-styles-wrapper .wp-block-quote', 'border-color', $accent, '' );
				wilbur_generate_css( '.has-accent-background-color, .editor-styles-wrapper .wp-block-button__link, .editor-styles-wrapper .wp-block-file__button', 'background-color', $accent );
			}

			// Background color.
			if ( $background && $background !== $background_default ) {
				wilbur_generate_css( '.editor-styles-wrapper', 'background-color', '#' . $background );
				wilbur_generate_css( '.has-background.has-primary-background-color:not(.has-text-color),.has-background.has-primary-background-color *:not(.has-text-color),.has-background.has-accent-background-color:not(.has-text-color),.has-background.has-accent-background-color *:not(.has-text-color)', 'color', '#' . $background );
			}
			...
			if ( $borders && $borders !== $borders_default ) {
				...
				wilbur_generate_css( '.editor-styles-wrapper .wp-block-table caption, .editor-styles-wrapper .wp-block-table.is-style-stripes tbody tr:nth-child(odd)', 'background-color', $borders );
			}

			// Text color.
			if ( $body && $body !== $body_default ) {
				wilbur_generate_css( 'body .editor-styles-wrapper, .editor-post-title__block .editor-post-title__input, .editor-post-title__block .editor-post-title__input:focus', 'color', $body );
			}
			...
			if ( $header_footer_background && $header_footer_background !== $header_footer_background_default ) {
				wilbur_generate_css( '.editor-styles-wrapper .wp-block-pullquote::before', 'background-color', $header_footer_background );
			}
			...
			// Accent color.
			if ( $accent && $accent !== $accent_default ) {
				wilbur_generate_css( 'body#tinymce.wp-editor.content a, body#tinymce.wp-editor.content a:focus, body#tinymce.wp-editor.content a:hover', 'color', $accent );
				wilbur_generate_css( 'body#tinymce.wp-editor.content blockquote, body#tinymce.wp-editor.content .wp-block-quote', 'border-color', $accent, '', ' !important' );
			...
			// Background color.
			if ( $background && $background !== $background_default ) {
				wilbur_generate_css( 'body#tinymce.wp-editor.content', 'background-color', '#' . $background );
			}

			// Text color.
			if ( $body && $body !== $body_default ) {
				wilbur_generate_css( 'body#tinymce.wp-editor.content', 'color', $body );
			}
			...
			// Borders color.
			if ( $borders && $borders !== $borders_default ) {
				wilbur_generate_css( 'body#tinymce.wp-editor.content pre, body#tinymce.wp-editor.content hr, body#tinymce.wp-editor.content fieldset,body#tinymce.wp-editor.content input, body#tinymce.wp-editor.content textarea', 'border-color', $borders );
			}
		}
