 ...
 */
function wilbur_site_logo( $args = array(), $echo = true ) {
	...
	/**
	 * Filters the arguments for `wilbur_site_logo()`.
	 *
	 * @param array  $args     Parsed arguments.
	 * @param array  $defaults Function's default arguments.
	 */
	$args = apply_filters( 'wilbur_site_logo_args', $args, $defaults );
	 ...
	 */
	$html = apply_filters( 'wilbur_site_logo', $html, $args, $classname, $contents );
 ...
 */
function wilbur_site_description( $echo = true ) {
	 ...
	 */
	$html = apply_filters( 'wilbur_site_description', $html, $description, $wrapper );
 ...
 * @return bool
 */
function wilbur_is_comment_by_post_author( $comment = null ) {
 ...
 */
function wilbur_filter_comment_reply_link( $link ) {
...
add_filter( 'comment_reply_link', 'wilbur_filter_comment_reply_link' );
 ...
 */
function wilbur_the_post_meta( $post_id = null, $location = 'single-top' ) {

	echo wilbur_get_post_meta( $post_id, $location ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped -- Escaped in wilbur_get_post_meta().
 ...
 */
function wilbur_edit_post_link( $link, $post_id, $text ) {
	if ( is_admin() ) {
		return $link;
	}
	...
	$text = sprintf(
		wp_kses(
			...
			__( 'Edit <span class="screen-reader-text">%s</span>', 'wilbur' ),
			array(
				'span' => array(
					'class' => array(),
				),
	...
	return '<div class="post-meta-wrapper post-meta-edit-link-wrapper"><ul class="post-meta"><li class="post-edit meta-wrapper"><span class="meta-icon">' . wilbur_get_theme_svg( 'edit' ) . '</span><span class="meta-text"><a href="' . esc_url( $edit_url ) . '">' . $text . '</a></span></li></ul><!-- .post-meta --></div><!-- .post-meta-wrapper -->';

}

add_filter( 'edit_post_link', 'wilbur_edit_post_link', 10, 3 );
 ...
 */
function wilbur_get_post_meta( $post_id = null, $location = 'single-top' ) {
	 ...
	 */
	$disallowed_post_types = apply_filters( 'wilbur_disallowed_post_types_for_meta_output', array( 'page' ) );
	...
	if ( 'single-top' === $location ) {
		...
		$post_meta = apply_filters(
			'wilbur_post_meta_location_single_top',
			array(
				'author',
				'post-date',
				'comments',
		 ...
		 */
		$post_meta = apply_filters(
			'wilbur_post_meta_location_single_bottom',
			array(
				'tags',
			)
		);
			...
			<ul class="post-meta<?php echo esc_attr( $post_meta_classes ); ?>">
				...
				if ( post_type_supports( get_post_type( $post_id ), 'author' ) && in_array( 'author', $post_meta, true ) && get_option( 'template_author_link' ) ) {
					...
					<li class="post-author meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Post author', 'wilbur' ); ?></span>
							<?php wilbur_the_theme_svg( 'user' ); ?>
						</span>
				...
				if ( in_array( 'post-date', $post_meta, true ) ) {
					...
					<li class="post-date meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Post date', 'wilbur' ); ?></span>
							<?php wilbur_the_theme_svg( 'calendar' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'categories', $post_meta, true ) && has_category() ) {
					...
					<li class="post-categories meta-wrapper">
						<span class="meta-icon">
							...
							<?php wilbur_the_theme_svg( 'folder' ); ?>
						</span>
						<span class="meta-text">
							<?php _ex( 'In', 'A string that is output before one or more categories', 'wilbur' ); ?> <?php the_category( ', ' ); ?>
						</span>
					</li>
					<?php
				...
				if ( in_array( 'tags', $post_meta, true ) && has_tag() ) {
					...
					<li class="post-tags meta-wrapper">
						<span class="meta-icon">
							<span class="screen-reader-text"><?php _e( 'Tags', 'wilbur' ); ?></span>
							<?php wilbur_the_theme_svg( 'tag' ); ?>
						</span>
						<span class="meta-text">
							<?php the_tags( '', ', ', '' ); ?>
						</span>
				...
				if ( in_array( 'comments', $post_meta, true ) && ! post_password_required() && ( comments_open() || get_comments_number() ) ) {
					...
					<li class="post-comment-link meta-wrapper">
						<span class="meta-icon">
							<?php wilbur_the_theme_svg( 'comment' ); ?>
						</span>
						<span class="meta-text">
				...
				if ( in_array( 'sticky', $post_meta, true ) && is_sticky() ) {
					...
					<li class="post-sticky meta-wrapper">
						<span class="meta-icon">
							<?php wilbur_the_theme_svg( 'bookmark' ); ?>
						</span>
						<span class="meta-text">
							<?php _e( 'Sticky post', 'wilbur' ); ?>
						</span>
					</li>
					<?php
				 ...
				 *                          Accepts 'single-top' or 'single-bottom'.
				 */
				do_action( 'wilbur_end_of_post_meta_list', $post_id, $post_meta, $location );
 ...
 */
function wilbur_filter_wp_list_pages_item_classes( $css_class, $page, $depth, $args, $current_page ) {
...
add_filter( 'page_css_class', 'wilbur_filter_wp_list_pages_item_classes', 10, 5 );
 ...
 */
function wilbur_add_sub_toggles_to_main_menu( $args, $item, $depth ) {
	...
	if ( isset( $args->show_toggles ) && $args->show_toggles ) {
		...
		if ( in_array( 'menu-item-has-children', $item->classes, true ) ) {
			...
			$args->after .= '<button class="toggle sub-menu-toggle fill-children-current-color" data-toggle-target="' . $toggle_target_string . '" data-toggle-type="slidetoggle" data-toggle-duration="' . absint( $toggle_duration ) . '" aria-expanded="false"><span class="screen-reader-text">' . __( 'Show sub menu', 'wilbur' ) . '</span>' . wilbur_get_theme_svg( 'chevron-down' ) . '</button>';
...
add_filter( 'nav_menu_item_args', 'wilbur_add_sub_toggles_to_main_menu', 10, 3 );
...
function wilbur_nav_menu_social_icons( $item_output, $item, $depth, $args ) {
	...
	if ( 'social' === $args->theme_location ) {
		$svg = \Wilbur\Classes\SVG_Icons::get_social_link_svg( $item->url );
		if ( empty( $svg ) ) {
...
add_filter( 'walker_nav_menu_start_el', 'wilbur_nav_menu_social_icons', 10, 4 );
...
add_action( 'wp_head', 'wilbur_no_js_class' );
 ...
 */
function wilbur_body_classes( $classes ) {
	...
	$background_color = get_theme_mod( 'background_color', 'fdfbe0' );
...
add_filter( 'body_class', 'wilbur_body_classes' );
 ...
 */
function wilbur_get_the_archive_title( $title ) {

	$regex = apply_filters(
		'wilbur_get_the_archive_title_regex',
		array(
			'pattern'     => '/(\A[^\:]+\:)/',
...
add_filter( 'get_the_archive_title', 'wilbur_get_the_archive_title' );
 ...
 */
function wilbur_toggle_duration() {
	/**
	 ...
	 */
	$duration = apply_filters( 'wilbur_toggle_duration', 250 );
 ...
 */
function wilbur_unique_id( $prefix = '' ) {
	static $id_counter = 0;
