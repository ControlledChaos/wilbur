...
class User_Colors {
	...
	public function tooltips() {
		...
		wp_add_inline_script(
			...
			'jQuery(document).ready(function($){
				$( ".wilbur-color-picker-tooltip" ).tooltip({
	...
	public function add_palettes() {
		...
		wp_admin_css_color( 'wilbur_light', _x( 'Custom Light', 'admin color scheme' ),
		...
		wp_admin_css_color( 'wilbur_dark', _x( 'Custom Dark', 'admin color scheme' ),
			get_theme_file_uri( "/assets/css/colors/light/colors$suffix.min.css" ),
		...
		wp_admin_css_color( 'wilbur_pink', _x( 'Pink', 'admin color scheme' ),
			...
			[ '#f0eddb', '#d7d2c5', '#b52712', '#5f1b29' ],
			[ 'base' => '#999999', 'focus' => '#cccccc', 'current' => '#cccccc' ]
		);

		wp_admin_css_color( 'wilbur_blue', _x( 'Blue', 'admin color scheme' ),
			get_theme_file_uri( "/assets/css/colors/blue/colors$suffix.min.css" ),
			[ '#eeeeee', '#bed2db', '#0073aa', '#074460' ],
			[ 'base' => '#999999', 'focus' => '#cccccc', 'current' => '#cccccc' ]
		);

		wp_admin_css_color( 'wilbur_violet', _x( 'Violet', 'admin color scheme' ),
		...
		wp_admin_css_color( 'wilbur_dark_pink', _x( 'Dark Pink', 'admin color scheme' ),
			...
			[ '#222222', '#ffffff', '#b52712', '#5f1b29' ],
			[ 'base' => '#999999', 'focus' => '#cccccc', 'current' => '#cccccc' ]
		);

		wp_admin_css_color( 'wilbur_dark_blue', _x( 'Dark Blue', 'admin color scheme' ),
			get_theme_file_uri( "/assets/css/colors/dark-blue/colors$suffix.min.css" ),
			[ '#222222', '#ffffff', '#0073aa', '#074460' ],
			[ 'base' => '#999999', 'focus' => '#cccccc', 'current' => '#cccccc' ]
		);

		wp_admin_css_color( 'wilbur_dark_violet', _x( 'Dark Violet', 'admin color scheme' ),
...
function admin_color_scheme_picker( $user_id ) {
	...
	if ( isset( $_wp_admin_css_colors['wilbur_light'] ) ) {
	...
	if ( empty( $current_color ) || ! isset( $_wp_admin_css_colors[ $current_color ] ) ) {
		$current_color = 'wilbur_light';
	}

	?>
	<div class="wilbur-color-picker">
		<div class="wilbur-color-message">
			...
			foreach ( $_wp_admin_css_colors as $color => $color_info ) :
				...
				<div class="color-option <?php echo ( $color == $current_color ) ? 'selected' : ''; ?>">
					...
					foreach ( $color_info->colors as $html_color ) {
						?>
						<li class="wilbur-color-picker-entry wilbur-color-picker-tooltip" style="background-color: <?php echo esc_attr( $html_color ); ?>" title="<?php echo esc_attr( __( 'Color hex code: ' ) . $html_color ); ?>"><span class="screen-reader-text"><?php echo esc_attr( __( 'Color hex code: ' ) . $html_color ); ?></span></li>
						<?php
					}
